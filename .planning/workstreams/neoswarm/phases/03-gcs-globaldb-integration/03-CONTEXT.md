...
- **D-22:** Current `GeniusElmInit(modelPath, knowledgePath)` sets `m_enableNetwork = false` (`genius_elm_chat_completions.cpp:158`). The init extension must be parameter-compatible: either a new init variant (e.g. `GeniusElmInitWithSwarm`) or added optional params — decided at plan time.
