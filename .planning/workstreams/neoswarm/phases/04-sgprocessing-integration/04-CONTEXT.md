...
- **D-13 (added post-research, 2026-08-18):** `InputFormat::FP4_ULTRA` targets **MNN_Ultra's E2M1 micro-float encoding** (the `origin/MNN_Ultra` branch's `VulkanFP4Dequant`/`fp4_dequant.comp` kernel) — confirmed by the user after research surfaced three incompatible "FP4" implementations across the ecosystem. NEO-SWARM's own `fp4::FP4Codec` (`src/core/fp4/fp4_codec.hpp`, NF4-style LUT) is **not** the target format — the user confirmed it predates this work and is orphaned/incorrect ("created by subaskar, most certainly nonsense"). The `poc` workstream's separate "SGFP4 v2" Python exporter is unrelated to this phase — the user's understanding is that it exports to ONNX for `mnnconvert`, a different pipeline concern, not an alternative FP4_ULTRA wire format. Flag `fp4_codec.hpp`/`.cpp` as dead code for removal or explicit re-justification during this phase (do not build PROC-02 around it).
