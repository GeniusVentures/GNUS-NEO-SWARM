...
namespace sgns::neoswarm::network
{
    ...
    outcome::result<void> P2PNode::Start()
    {
        ...
        try
        {
            ...
            auto key_marshaller = injector.template create<std::shared_ptr<libp2p::crypto::marshaller::KeyMarshaller>>();
            impl_->gossip_ = libp2p::protocol::gossip::create(
                scheduler,
                impl_->host_,
                impl_->id_mgr_,
                crypto_provider,
                key_marshaller,
                libp2p::protocol::gossip::Config{} );
                ...
                [this]( libp2p::protocol::gossip::Gossip::SubscriptionData sub_data )
                {
                    if ( sub_data && task_handler_ )
                    {
                        ...
                        auto json = nlohmann::json::parse(
                            std::string( msg.data.begin(), msg.data.end() ), nullptr, false );
            ...
            impl_->subs_->crdt_sub = impl_->gossip_->subscribe(
                { kCRDTTopic },
                [this]( libp2p::protocol::gossip::Gossip::SubscriptionData sub_data )
                {
            ...
            auto listen_ma = libp2p::multi::Multiaddress::create(
                cfg_.listen_addr_.empty() ? "/ip4/0.0.0.0/tcp/0" : cfg_.listen_addr_ );
            if ( listen_ma )
            {
...
#ifdef GENIUS_HAS_LIBP2P
        if ( impl_->gossip_ ) impl_->gossip_->stop();
        if ( impl_->host_ )   impl_->host_->stop();
        impl_->host_.reset();
        impl_->gossip_.reset();
        impl_->id_mgr_.reset();
#endif
