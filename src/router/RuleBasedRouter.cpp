...
namespace sgns::neoswarm::router
{
    ...
    RuleBasedRouter::RuleBasedRouter() : cfg_( {} ) {}
    RuleBasedRouter::RuleBasedRouter( Config cfg ) : cfg_( std::move( cfg ) ) {}

    // -----------------------------------------------------------------------
    // SelectMode
    // -----------------------------------------------------------------------
    ExecutionMode RuleBasedRouter::SelectMode( const PromptFeatures &features,
    ...
    outcome::result<RouteDecision> RuleBasedRouter::Route( const Task &task )
    {
        ...
        RouterLogger()->debug( "Route: target={} mode={} confidence={:.2f} reason='{}'",
                               static_cast<int>( decision.target_ ),
                               static_cast<int>( decision.mode_ ),
                               decision.confidence_,
                               decision.reasoning_ );
